...
class PlotCanvas(QMainWindow):
    def __init__(self, img, parent):
        ...
        self._scale, self._units, self._unitsPower, dimension = self.parse_scale_units(
            scale, units
        )
        ...
        # Process history
        temcompanion_info = {
            "Version": f"TemCompanion v{self.ver}",
    ...
    def parse_scale_units(self, scale, units):
        ...
        if units in real_units_list:
            ...
            si_scale = fn.siEval(
                f"{scale} {real_units}", unitPower=-1
            )  # This converts to 1/meters
            unitsPower = -1
    ...
    def create_scalebar(self):
        ...
        self.scalebar = CustomScaleBar(
            parent=self.canvas.viewbox, units=self.units, power=self._unitsPower
        )
    ...
    def get_scalebar_length(self):
        ...
        if self.attribute["dimension"] == "px-length":
            ...
            scale_float_preferred, units_preferred = fn.siFormat(
                scale_dx_float, suffix="m", power=self._unitsPower, precision=3
            ).split(" ")

        # Set the length to 1, 2, 5, 10, 20, 50, 100, 200, 500.
        scale_dx_list = [1, 2, 5, 10, 20, 50, 100, 200, 500]
        scale_dx = min(
            scale_dx_list, key=lambda x: abs(x - float(scale_float_preferred))
        )
        # Convert back to meters
        if self.attribute["dimension"] != "px-length":
            scale_dx = fn.siEval(
                f"{scale_dx} {units_preferred}", unitPower=self._unitsPower
            )
    ...
    def update_scalebar(self):
        ...
        self.scalebar.set_properties(
            font_size=font_size, color=color, location=location
        )
        self.scalebar.toggle_visibility(self.attribute["scalebar"])
    ...
    def setscale(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.canvas.viewbox.setRange(
                xRange=(0, self.img_size[-1] * self.scale),
                yRange=(0, self.img_size[-2] * self.scale),
                padding=0,
            )
        ...
        if self.canvas.image_item:
            ...
            if 0 <= int(img_x) < img_size[-1] and 0 <= int(img_y) < img_size[-2]:
                ...
                self.pixel_label.setText(
                    f"{x_label} | {img_x:.1f} px, {y_label} | {img_y:.1f} px, {val_label}"
                )
    ...
    def new_img_from_display(self):
        ...
        self.plot_new_image(
            img_dict, canvas_name, parent=self.parent(), position="center"
        )

    def plot_new_image(
        self, img_dict, canvas_name, parent=None, metadata=None, position=None
    ):
    ...
    def copy_img(self):
        ...
        pixmap = (
            self.canvas.viewbox.scene()
            .views()[0]
            .grab(self.canvas.viewbox.sceneBoundingRect().toRect())
        )
        ...
        self.statusBar.showMessage(
            "The current image has been copied to the clipboard!"
        )
    ...
    def rotate(self):
        ...
        self.buttons["cancel"].triggered.connect(
            lambda: self.clean_up(selector=True, buttons=True, status_bar=True)
        )
    ...
    def display_rotation_angle(self):
        if self.canvas.selector:
            ...
            start_point, end_point = (
                selector.getHandles()[0].pos(),
                selector.getHandles()[1].pos(),
            )
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "Input Error", "Please enter a valid rescale factor."
                )
                return
            ...
            self.plot_new_image(
                ...
                preview_name,
                parent=self.parent(),
                metadata=f"Resampled {title} by a factor of {rescale_factor}.",
                position="center right",
            )
    ...
    def simplemath(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                QMessageBox.warning(
                    self, "Simple Math", f"Operation not possible on image 1: {e}"
                )
                return
            try:
                ...
                QMessageBox.warning(
                    self, "Simple Math", f"Operation not possible on image 2: {e}"
                )
                return
            ...
            if operation == "Add":
                try:
                    ...
                    QMessageBox.warning(
                        self, "Simple Math", f"Operation not possible: {e}"
                    )
                    return
    ...
    def wiener_filter(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Invalid Parameters",
                "Please enter valid numbers for the filter parameters.",
            )
            return
        ...
        if apply_to is not None:
            ...
            print(
                f"Applying Wiener filter to {title} with delta = {delta_wf}, Bw-order = {order_wf}, Bw-cutoff = {cutoff_wf}..."
            )
            self.worker = Worker(
                apply_filter_on_img_dict,
                img_wf,
            ...
            self.worker.finished.connect(
                lambda: print(
                    f"Applied Wiener filter to {title} with delta = {delta_wf}, Bw-order = {order_wf}, Bw-cutoff = {cutoff_wf}."
                )
            )
            self.worker.result.connect(
                lambda result: self.plot_new_image(
                    result,
                    preview_name,
                    parent=self.parent(),
                    metadata=metadata,
    ...
    def absf_filter(self):
        ...
        if apply_to is not None:
            ...
            # Apply the filter in a separate thread
            print(
                f"Applying ABS filter to {title} with delta = {delta_absf}, Bw-order = {order_absf}, Bw-cutoff = {cutoff_absf}..."
            )
            self.worker = Worker(
                apply_filter_on_img_dict,
                img_absf,
            ...
            self.worker.finished.connect(
                lambda: print(
                    f"Applied ABS filter to {title} with delta = {delta_absf}, Bw-order = {order_absf}, Bw-cutoff = {cutoff_absf}."
                )
            )
            self.worker.result.connect(
                lambda result: self.plot_new_image(
    ...
    def non_linear_filter(self):
        ...
        if apply_to is not None:
            ...
            # Apply the filter in a separate thread
            print(
                f"Applying Non-Linear filter to {title} with N = {N}, delta = {delta_nl}, Bw-order = {order_nl}, Bw-cutoff = {cutoff_nl}..."
            )
            self.worker = Worker(
                apply_filter_on_img_dict,
                img_nl,
            ...
            self.worker.finished.connect(
                lambda: print(
                    f"Applied Non-Linear filter to {title} with N = {N}, delta = {delta_nl}, Bw-order = {order_nl}, Bw-cutoff = {cutoff_nl}."
                )
            )
            self.worker.result.connect(
                lambda result: self.plot_new_image(
    ...
    def bw_filter(self):
        ...
        if apply_to is not None:
            ...
            # Apply the filter in a separate thread
            print(
                f"Applying Butterworth filter to {title} with Bw-order = {order_bw}, Bw-cutoff = {cutoff_bw}..."
            )
            self.worker = Worker(
                apply_filter_on_img_dict,
                img_bw,
            ...
            self.worker.finished.connect(
                lambda: print(
                    f"Applied Butterworth filter to {title} with Bw-order = {order_bw}, Bw-cutoff = {cutoff_bw}."
                )
            )
            self.worker.result.connect(
                lambda result: self.plot_new_image(
    ...
    def gaussian_filter(self):
        ...
        if apply_to is not None:
            preview_name = (
                self.canvas.canvas_name + "_" + apply_to + "_Gaussian Filtered"
            )
            # Tailor metadata based on high-pass cutoff
            if (
                hp_cutoff_gaussian <= 0 or hp_cutoff_gaussian >= 1
            ):  # No high-pass filter
                metadata = (
                    f"Gaussian low-pass filter applied with cutoff = {cutoff_gaussian}"
                )
            ...
            self.worker = Worker(
                ...
                hp_cutoff_ratio=hp_cutoff_gaussian,
            )
            self.toggle_progress_bar("ON")
            self.worker.finished.connect(lambda: self.toggle_progress_bar("OFF"))
            self.worker.finished.connect(self.worker.deleteLater)
    ...
    def show_info(self):
        ...
        # Add some extra info
        img_info["TemCompanion"]["Image Size (pixels)"] = (
            f"{self.canvas.img_size[-1]} x {self.canvas.img_size[-2]}"
        )
        img_info["TemCompanion"]["Calibrated Image Size"] = (
            f"{self.canvas.img_size[-1] * self.scale:.4g} x {self.canvas.img_size[-2] * self.scale:.4g} {self.units}"
        )
    ...
    def fft(self):
        ...
        if img_dict["data"].ndim != 2:
            QMessageBox.warning(
                self, "FFT Warning", "FFT can only be performed on 2D grayscale images!"
            )
            return
    ...
    def live_fft(self, fullsize=False, windowed=False, resize_fft=False):
        if self.canvas.current_img.ndim != 2:
            QMessageBox.warning(
                self,
                "Live FFT Warning",
                "Live FFT can only be performed on 2D grayscale images!",
            )
            return
        self.clean_up(
            selector=True, buttons=True, modes=True, status_bar=True
        ...
        x0, y0 = (
            int(selector.pos().x() / self.scale),
            int(selector.pos().y() / self.scale),
        )
        ...
        selector.sigRegionChanged.connect(
            lambda: self.update_live_fft(windowed=windowed, resize_fft=resize_fft)
        )
    ...
    def update_live_fft(self, windowed=False, resize_fft=False):
        if self.canvas.selector and self.mode_control["Live_FFT"]:
            ...
            if preview_name in self.parent().preview_dict:
                ...
                print(
                    f"Displaying live FFT of {self.canvas.canvas_name} from {x0}:{x1}, {y0}:{y1}."
                )
    ...
    def display_distance(self):
        if self.canvas.selector and self.mode_control["measurement"]:
            ...
            self.statusBar.showMessage(
                f"Measurement: {distance:.3f} {self.units}, {angle:.2f}°"
            )
    ...
    def measure_angle(self):
        ...
        selector.sigRegionChanged.connect(
            lambda: self.statusBar.showMessage(
                f"Angle Measurement: {selector.angle:.2f}°"
            )
        )
        self.canvas.setFocus()  # Ensure the canvas has focus to receive key events
    ...
    def lineprofile(self):
        ...
        line_profile_window = PlotCanvasSpectrum(
            line_x, line_profile, self.canvas.canvas_name, parent=self.parent()
        )
    ...
    def extract_line_profile(self):
        if self.canvas.selector and self.mode_control["lineprofile"]:
            ...
            x0, y0 = (
                int(start_point.x() / self.scale),
                int(start_point.y() / self.scale),
            )
            ...
            line_profile = profile_line(
                data, (y0, x0), (y1, x1), linewidth=width, reduce_func=np.mean
            )
            line_x = (
                np.linspace(0, len(line_profile) - 1, len(line_profile)) * self.scale
            )
    ...
    def radial_integration(self):
        ...
        self.statusBar.showMessage(
            f"Center: {self.img_size[-1] // 2}, {self.img_size[-2] // 2}"
        )
    ...
    def perform_radial_integration(self):
        ...
        print(
            f"Performed radial integration on {self.windowTitle()} from the center: {center}."
        )
    ...
    def update_radial_integration_center(self):
        if self.canvas.selector and self.mode_control["radial_integration"]:
            ...
            px_center_x, px_center_y = (
                int(center_x / self.scale),
                int(center_y / self.scale),
            )
            # Run CoM to get more accurate center
            image_data = self.canvas.current_img
            window_size = int(radius / self.scale)  # in pixels
            cx_pix, cy_pix = refine_center(
                image_data, (px_center_x, px_center_y), window_size
            )
            cx_pix = int(cx_pix)
            cy_pix = int(cy_pix)
            ...
            selector.sigRegionChangeFinished.disconnect(
                self.update_radial_integration_center
            )
            selector.setPos([x, y])
    ...
    def measure_fft(self):
        ...
        self.statusBar.showMessage(
            "Drag the circle over spots to measure. Define center as needed."
        )
        ...
        self.buttons["define_center"] = QAction(
            QIcon(center_icon), "Define Center", parent=self
        )
        self.buttons["define_center"].setStatusTip("Define the center of FFT")
        self.buttons["define_center"].triggered.connect(
            lambda: self.define_center_dp(method="center")
        )
        self.toolbar.addAction(self.buttons["define_center"])
        center2_icon = os.path.join(self.wkdir, "icons/dp_center2.png")
        self.buttons["define_center2"] = QAction(
            QIcon(center2_icon), "Define Center with Two Points", parent=self
        )
        self.buttons["define_center2"].setStatusTip(
            "Define the FFT center by two symmetric points"
        )
        self.buttons["define_center2"].triggered.connect(
            lambda: self.define_center_dp(method="two-point")
        )
        ...
        selector.addTranslateHandle(
            [0.5, 0.5]
        )  # Add a handle at the center for easier dragging
    ...
    def calculate_fft_distance(self):
        if self.canvas.selector and self.mode_control["measure_fft"]:
            ...
            self.statusBar.showMessage(
                f"FFT Measurement: {distance:.3f} {self.real_units}, {angle:.2f}°"
            )
    ...
    def define_center_dp(self, method="center"):
        ...
        if method == "center":
            ...
            selector1.addTranslateHandle(
                [0.5, 0.5]
            )  # Add a handle at the center for easier dragging
            selector2 = pg.CircleROI(
                [x0_2, y0_2],
                radius=window_size,
                pen=pg.mkPen("r", width=3),
                movable=True,
    ...
    def accept_define_center(self):
        ...
        if self.temp_center:
            if len(self.temp_center) == 1:
                ...
                self.canvas.center = (
                    (self.temp_center[0][0] + self.temp_center[1][0]) / 2,
    ...
    def show_crosshair(self):
        ...
        hLine = self.canvas.plot.plot(
            [cx - line_length, cx + line_length], [cy, cy], pen=pg.mkPen("g", width=3)
        )
        vLine = self.canvas.plot.plot(
            [cx, cx], [cy - line_length, cy + line_length], pen=pg.mkPen("g", width=3)
        )
    ...
    def gpa(self):
        ...
        self.canvas.selector[0].setSize(
            (self.img_size[-1] * self.scale, self.img_size[-2] * self.scale)
        )
        preview_name = self.canvas.canvas_name + "_Live FFT"
        ...
        # Buttons
        ok_icon = os.path.join(self.wkdir, "icons/OK.png")
        preview_fft.buttons["ok"] = QAction(
            QIcon(ok_icon), "Run GPA", parent=preview_fft
        )
        ...
        cancel_icon = os.path.join(self.wkdir, "icons/cancel.png")
        preview_fft.buttons["cancel"] = QAction(
            QIcon(cancel_icon), "Close", parent=preview_fft
        )
        ...
        preview_fft.buttons["settings"] = QAction(
            QIcon(settings_icon), "GPA Settings", parent=preview_fft
        )
        ...
        preview_fft.buttons["add_mask"] = QAction(
            QIcon(add_icon), "Add Mask", parent=preview_fft
        )
        preview_fft.buttons["add_mask"].setStatusTip("Add Mask")
        preview_fft.toolbar.addAction(preview_fft.buttons["add_mask"])
        preview_fft.buttons["add_mask"].triggered.connect(
            lambda: preview_fft.add_mask(
                r=self.attribute["gpa"]["mask_r"] * preview_fft.scale, pairs=False
            )
        )
        remove_icon = os.path.join(self.wkdir, "icons/minus.png")
        preview_fft.buttons["remove_mask"] = QAction(
            QIcon(remove_icon), "Remove Mask", parent=preview_fft
        )
        ...
        preview_fft.buttons["refine_center"] = QAction(
            QIcon(refine_icon), "Refine Center", parent=preview_fft
        )
        preview_fft.buttons["refine_center"].setStatusTip(
            "Refine the mask positions using CoM."
        )
    ...
    def stop_gpa(self):
        preview_name = self.canvas.canvas_name + "_Live FFT"
        self.parent().preview_dict[preview_name].clean_up(
            selector=True, buttons=True, modes=True, status_bar=True
        )
    ...
    def run_gpa(self):
        ...
        self.worker.finished.connect(self.worker.deleteLater)
        self.worker.finished.connect(
            lambda: print(f"Finished running GPA on {title} with {algorithm} GPA.")
        )
    ...
    def display_gpa_result(self, result):
        ...
        main_window.preview_dict[preview_name_exx].canvas.image_item.setLevels(
            (vmin, vmax)
        )
        ...
        main_window.preview_dict[preview_name_eyy].canvas.image_item.setLevels(
            (vmin, vmax)
        )
        ...
        main_window.preview_dict[preview_name_exy].canvas.image_item.setLevels(
            (vmin, vmax)
        )
        ...
        main_window.preview_dict[preview_name_oxy].canvas.image_item.setLevels(
            (vmin, vmax)
        )
    ...
    def gpa_settings(self):
        ...
        dialog = gpaSettings(
            r_pix, edgesmooth, step, sigma, algorithm, vmin=vmin, vmax=vmax, parent=self
        )
        if dialog.exec_() == QDialog.Accepted:
    ...
    def refine_mask(self):
        ...
        for mask in preview_fft.canvas.selector:
            ...
            cx, cy = (
                g_refined[0] * preview_fft.scale - radius,
                g_refined[1] * preview_fft.scale - radius,
            )
            mask.setPos([cx, cy])
    ...
    def confirm_rotate_stack(self):
        if self.canvas.selector:
            ...
            self.plot_new_image(
                ...
                preview_name,
                parent=self.parent(),
                metadata=f"Rotated the entire stack of {title} by {angle:.1f} degrees counterclockwise.",
                position="center right",
            )

            print(
                f"Rotated the entire stack of {title} by {angle:.1f} degrees counterclockwise."
            )
            self.position_window("center left")
    ...
    def manual_rotate_stack(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.plot_new_image(
                ...
                preview_name,
                parent=self.parent(),
                metadata=f"Rotated the entire stack of {title} by {ang:.1f} degrees counterclockwise.",
                position="center right",
            )

            print(
                f"Rotated the entire stack of {title} by {ang:.1f} degrees counterclockwise."
            )
            self.position_window("center left")
    ...
    def run_alignment_cc(
        ...
        for n in range(img_n - 1):
            ...
            drift, _, _ = phase_cross_correlation(
                fixed, moving, upsample_factor=upsampling, normalization=normalization
            )
            # Correct the drift
    ...
    def align_stack_of(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            # Run alignment in a separate thread
            self.worker = Worker(
                self.run_alignment_of, img, window_size, prefilter, gaussian
            )
            self.toggle_progress_bar("ON")
            self.worker.finished.connect(lambda: self.toggle_progress_bar("OFF"))
            self.worker.finished.connect(self.worker.deleteLater)
    ...
    def run_alignment_of(
        self, img_dict, window_size=20, prefilter=True, gaussian=False
    ):
        aligned_img = copy.deepcopy(img_dict)
    ...
    def integrate_stack(self):
        ...
        integrated_img = {
            "data": data,
            "axes": self.canvas.data["axes"],
            "metadata": self.canvas.data["metadata"],
    ...
    def export_stack(self):
        ...
        options = QFileDialog.Options()
        file_path, selection = QFileDialog.getSaveFileName(
            self.parent(),
            "Save as tiff stack",
            "",
            "TIFF Files (*.tiff)",
    ...
    def export_stack_gif(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.worker.finished.connect(self.worker.deleteLater)
            self.worker.finished.connect(
                lambda: print(f"{file_path} has been exported.")
            )
            self.worker.start()
    ...
    def export_series(self):
        ...
        options = QFileDialog.Options()
        file_path, selected_type = QFileDialog.getSaveFileName(
            self.parent(),
            "Save Figure",
            "",
        ...
        if file_path:
            ...
            if selected_type == "16-bit TIFF Files (*.tiff)":
                for i in range(img_to_save["data"].shape[0]):
                    img = {
                        "data": img_to_save["data"][i],
                        "axes": img_to_save["axes"],
                        "metadata": img_to_save["metadata"],
                    }
                ...
                for i in range(img_to_save["data"].shape[0]):
                    ...
                    save_as_tif16(
                        img, f_name + f"_{i:03d}", output_dir, dtype="float32"
                    )
                    print(f"Exported to {output_dir}/{f_name}_{i:03d}.")

            elif selected_type in [
                "8-bit TIFF Files (*.tiff)",
                "Grayscale PNG Files (*.png)",
                "Grayscale JPEG Files (*.jpg)",
            ]:
                for i in range(img_to_save["data"].shape[0]):
                    ...
                    save_with_pil(
                        ...
                        output_dir,
                        file_type,
                        scalebar=self.scalebar_settings["scalebar"],
                    )
                    print(f"Exported to {output_dir}/{f_name}_{i:03d}.")
...
class PlotCanvasFFT(PlotCanvas):
    def __init__(self, img, source_img, parent=None):
        ...
        self.attribute["complex_display"] = (
            "Magnitude"  # Default display mode for complex FFT data
        )
        ...
        self.canvas.create_img(
            cmap=self.canvas.attribute["cmap"],
    ...
    def calculate_fft(self):
        ...
        self.canvas.data["fft"] = (
            fft_data  # Store the complex FFT data for later use in masking and iFFT
        )
        self.canvas.data["data"] = (
            fft_mag  # 'data' is what is used for display, so we keep the magnitude here
        )
    ...
    def mask(self):
        ...
        self.buttons["add"].triggered.connect(
            lambda: self.add_mask(r=mask_r, pairs=True)
        )
        ...
        for axes in live_ifft["axes"]:
            ...
            axes["offset"] = 0
        live_ifft["metadata"]["TemCompanion"]["Pixel Calibration"] = (
            f"{img_scale:.4g} {img_units}"
        )
        live_ifft["metadata"]["TemCompanion"]["Image Size (pixels)"] = (
            f"{live_ifft['data'].shape[1]} x {live_ifft['data'].shape[0]}"
        )
        live_ifft["metadata"]["TemCompanion"]["Calibrated Image Size"] = (
            f"{live_ifft['data'].shape[1] * img_scale:.4g} x {live_ifft['data'].shape[0] * img_scale:.4g} {img_units}"
        )
    ...
    def add_mask(self, r, pairs=True):
        ...
        mask0.addTranslateHandle(
            [0.5, 0.5]
        )  # Add a handle at the center for easier movement
        mask0_id = id(mask0)
        mask0.id = mask0_id
    ...
    def ifft_with_masks(self, fft_data):
        ...
        for m in self.canvas.selector:
            m_center = (
                (m.pos().x() + m.size()[0] / 2) / self.scale,
                (m.pos().y() + m.size()[1] / 2) / self.scale,
            )
...
class PlotCanvasSpectrum(QMainWindow):
    def __init__(self, x, y, source_img=None, parent=None):
        ...
        self.selector = None
        self.buttons = {"ok": None, "cancel": None}
    ...
    def create_plot(self, title=None, xlabel=None, ylabel=None):
        self.plot_data_item = self.plot.plot(
            self.x, self.y, pen=pg.mkPen("blue", width=2)
        )
    ...
    def measure(self, orientation="horizontal"):
        ...
        self.selector = pg.LinearRegionItem(
            orientation=orientation, movable=True, swapMode="block"
        )
