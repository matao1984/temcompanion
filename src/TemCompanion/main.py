...
from .functions import (load_file, getDirectory, getFileNameType, save_as_tif16, save_with_pil,
...
class PlotCanvas(QMainWindow):
    def __init__(self, img, parent):
        ...
        if 'TemCompanion' in img['metadata']:
            ...
            self.process = {'Version': f'TemCompanion v{ver}', 
    ...
    def plot_new_image(self, img_dict, canvas_name, parent=None, metadata=None, position=None):
        ...
        if position is not None:
            UI_TemCompanion.preview_dict[canvas_name].position_window(position)
        
        return UI_TemCompanion.preview_dict[canvas_name]
    ...
    def simplemath(self):
        img_list = UI_TemCompanion.preview_dict
    ...
    def fft(self):
        ...
        UI_TemCompanion.preview_dict[preview_name] = PlotCanvasFFT(img_dict, title, parent=self.parent())
    ...
    def live_fft(self, fullsize=False, windowed=False, resize_fft=False):
        ...
        UI_TemCompanion.preview_dict[preview_name] = PlotCanvasFFT(self.live_img, title, parent=self.parent())
        UI_TemCompanion.preview_dict[preview_name].setWindowTitle(preview_name)
        UI_TemCompanion.preview_dict[preview_name].canvas.canvas_name = preview_name
        UI_TemCompanion.preview_dict[preview_name].show()
    ...
    def radial_integration(self):
        ...
        if self.radial_integration_dialog.exec_() == QDialog.Accepted:
            ...
            x_label = f'Radial Distance ({self.unit})'
            y_label = 'Integrated Intensity (Counts)'
            plot = PlotCanvasSpectrum(x_data, y_data, parent=self)
...
class PlotCanvasFFT(PlotCanvas):
    ...
    def closeEvent(self, event):  
        source_canvas = find_img_by_title(UI_TemCompanion.preview_dict, self.source_img)     
    ...
    def mask(self):
        ...
        self.plot_new_image(live_ifft, preview_name, parent=self.parent(), metadata=metadata)
        self.position_window('center left')
        UI_TemCompanion.preview_dict[preview_name].position_window('center right')
