from PyQt5.QtWidgets import QAction, QDialog, QToolBar, QFileDialog
from PyQt5.QtCore import Qt,  QThread, QRectF, QSize, QPointF, pyqtSignal, pyqtSlot
...
class DiffractionCanvas(PlotCanvas):
    def __init__(self, img4d, master_handle, parent=None):
        ...
        self.canvas.update_img(q_img_data, pvmin=self.pvmin, pvmax=self.pvmax)  # Update the image with the specified pvmin and pvmax for 4D-STEM diffraction
    ...
    def closeEvent(self, event):
        ...
        if R_canvas_name in self.parent().preview_dict:
            self.parent().preview_dict[R_canvas_name].close() # Close the virtual image canvas if it's still open
    ...
    def create_toolbar(self):
        ...
        point_detector_action = QAction(QIcon(point_detector_icon), "Point Detector", self)
        ...
        circle_detector_action = QAction(QIcon(circle_detector_icon), "Circle Detector", self)
        circle_detector_action.setStatusTip("Add a circle detector for the virtual image")
        ...
        annular_detector_action = QAction(QIcon(annular_detector_icon), "Annular Detector", self)
        annular_detector_action.setStatusTip("Add an annular detector for the virtual image")
...
class VirtualImageCanvas(PlotCanvas):
    ...
    def closeEvent(self, event):
        ...
        if Q_canvas_name in self.parent().preview_dict:
            self.parent().preview_dict[Q_canvas_name].close() # Close the diffraction canvas if it's still open
    ...
    def create_toolbar(self):
        ...
        rectangle_detector_icon = os.path.join(self.wkdir, "icons/rectangle_detector.png")
        rectangle_detector_action = QAction(QIcon(rectangle_detector_icon), "Rectangle Detector", self)
        rectangle_detector_action.setStatusTip("Add a rectangle detector for mean diffraction")
...
class PlotCanvas4D:
    def __init__(self, img, parent=None):
        ...
        self.R_canvas = VirtualImageCanvas(self.img4d, master_handle=self, parent=parent)
    ...
    def update_point_detector_diffraction(self):
        selector = self.Q_canvas.canvas.selector[0]  # Assuming only one selector for point detector
        pos = selector.pos() + selector.size() * 0.5  # Get the center position of the circle ROI
    ...
    def update_point_detector_virtualimg(self):
        selector = self.R_canvas.canvas.selector[0]  # Assuming only one selector for point detector
        pos = selector.pos() + selector.size() * 0.5  # Get the center position of the circle ROI
    ...
    def rectangle_detector_virtualimg(self):
        ...
        selector = pg.RectROI([x0 - width/2, y0 - height/2], [width, height],
    ...
    def update_rectangle_detector_virtualimg(self):
        ...
        q_img_data = np.mean(self.img_data[y_start:y_end, x_start:x_end, :, :], axis=(0,1))
        pvmin = self.Q_canvas.pvmin
        pvmax = self.Q_canvas.pvmax
        self.Q_canvas.canvas.update_img(q_img_data, pvmin=pvmin, pvmax=pvmax)
    ...
    def circle_detector_diffraction(self, function):
        ...
        x0 = self.Q_center[1] * self.Q_canvas.scale
        y0 = self.Q_center[0] * self.Q_canvas.scale
        radius = 0.05 * min(x_range, y_range)  # Default radius as 5% of the smaller dimension of the canvas
        selector = pg.CircleROI([x0-radius, y0-radius], radius=radius,
                                    pen=pg.mkPen('yellow', width=5),
                                    movable=True,
                                    rotatable=False,
                                    resizable=True,
    ...
    def update_detector_diffraction(self):
        ...
        for selector in self.Q_canvas.canvas.selector:
            pos = selector.pos() + selector.size() * 0.5  # Get the center position of the circle ROI
            radius = int(selector.size().x() / 2 / self.Q_canvas.scale)  # Assuming the size is the diameter of the circle
    ...
    def annular_detector_diffraction(self, function):
        ...
        selector = AnnularROI([x0, y0], inner_radius=inner_radius, outer_radius=outer_radius,
                                    pen=pg.mkPen('yellow', width=5),
                                    movable=True,
                                    rotatable=False,
                                    resizable=True,
    ...
    def get_annular_mask(self, size, center, inner_radius, outer_radius):
        ...
        mask[(dist_from_center >= inner_radius) & (dist_from_center <= outer_radius)] = 1
        return mask

    def update_annular_detector_diffraction(self):
        # Update the virtual image with all selectors
        selector = self.Q_canvas.canvas.selector[0]  # Assuming only one selector for annular detector
    ...
    def update_com(self, signal='CoM'):
        ...
        self.worker = Worker(self.calculate_com, self.img_data, (x_center, y_center), radius, mode=signal)
        self.R_canvas.toggle_progress_bar('ON')
        self.worker.finished.connect(lambda: self.R_canvas.toggle_progress_bar('OFF'))
        self.worker.finished.connect(self.worker.deleteLater)
        self.worker.result.connect(self._on_com_result)
    ...
    def calculate_com(self, data, center, radius, mode='CoM'):
        ...
        if isinstance(radius, (int, float)):
            mask = create_mask(Q_size, [(int(x_center), int(y_center))], [radius], edge_blur=0).astype(np.float64, copy=False)
        elif isinstance(radius, (tuple, list)) and len(radius) == 2:
            mask = self.get_annular_mask(Q_size, center, radius[0], radius[1]).astype(np.float64, copy=False)
    ...
    def update_dpc(self, signal='CoM'):
        ...
        # Calculate CoM with a separate thread
        self.worker = Worker(self.calculate_com, self.img_data, (x_center, y_center), (inner_radius_px, outer_radius_px), mode=signal)
        self.R_canvas.toggle_progress_bar('ON')
        self.worker.finished.connect(lambda: self.R_canvas.toggle_progress_bar('OFF'))
        self.worker.finished.connect(self.worker.deleteLater)
        self.worker.result.connect(self._on_com_result)
    ...
    def crop_R(self):
        ...
        selector = self.R_canvas.canvas.selector[0]  # Assuming only one selector for cropping
        pos = selector.pos()  # Get the top-left position of the rectangle ROI
        size = selector.size()  # Get the size of the rectangle ROI
        ...
        self.R_canvas.update_metadata('Real space cropped to x:[{}:{}], y:[{}:{}]'.format(x_start, x_end, y_start, y_end))
    ...
    def crop_Q(self):
        ...
        selector = self.Q_canvas.canvas.selector[0]  # Assuming only one selector for cropping
        pos = selector.pos()  # Get the top-left position of the rectangle ROI
        size = selector.size()  # Get the size of the rectangle ROI
        ...
        # Update the process history
        self.Q_canvas.update_metadata('Reciprocal space cropped to qx:[{}:{}], qy:[{}:{}]'.format(x_start, x_end, y_start, y_end))
...
class AnnularROI(pg.CircleROI):
    ...
    def __init__(self, center, inner_radius, outer_radius, **kwargs):
        ...
        pos_inner = (self.center[0] - self.inner_radius, self.center[1] - self.inner_radius)
        ...
        self.outer_circle.sigRegionChangeFinished.connect(self._on_outer_change_finished)
    ...
    def update_outer_circle(self):
        ...
        try:
            ...
            self.outer_circle.setSize((2.0 * self.outer_radius, 2.0 * self.outer_radius))
        finally:
    ...
    def update_inner_from_outer(self):
        ...
        inner_radius = self.size().x() / 2.0
        center_local = QPointF(outer_pos.x() + outer_radius, outer_pos.y() + outer_radius)
        ...
        try:
            ...
            self.outer_circle.setPos(inner_radius - self.outer_radius, inner_radius - self.outer_radius)
            self.outer_circle.setSize((2.0 * self.outer_radius, 2.0 * self.outer_radius))
        finally:
